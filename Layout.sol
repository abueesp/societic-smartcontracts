...
// 3.1.1 bool
/// 3.1.1.1. logical operators: !, &&, || -Warn: || means OR, and in the expression f(x) || g(y), if f(x) evaluates to true, g(y) will not be evaluated even if it may have side-effects-, ==, =!

// 3.1.2 int/uint: u in unit means unsigned interger, both are have size equal to uint256 and int256, but they could also be uint8 and int8 f.i. WHAT IS BITWISE, WHAT IS UNARY AND WHAT MEANS Division always truncates (it just maps to the DIV opcode of the EVM), but it does not truncate if both operators are literals (or literal expressions, see points 7 and 8).
/// 3.1.2.1. comparative operators: <=, <, ==, !=, >=, >
/// 3.1.2.2. bit operators: &, |, ^ (bitwise exclusive or), ~ (bitwise negation)
/// 3.1.2.3. arithmetic operators: +, -, unary -, unary +, *, /, % (remainder), ** (exponentiation)
...
//3.1.4.1. Data location: arrays and structs, has an additional annotation, the “data location”, about whether it is stored in memory or in storage.
...
//3.1.4.1.1.Forced data location:
...
//3.1.4.1.2.Default data location:
...
///3.1.4.2.1.For storage arrays, the element type can be arbitrary (i.e. also other arrays, mappings or structs).
////Allocation: It is possible to resize memory storage arrays by assigning to the .length member.
///3.1.4.2.2.For memory arrays, it cannot be a mapping and has to be an ABI type if it is an argument of a publicly-visible function. 
...
///3.1.4.2.3.Variables of type bytes and string are special arrays. 
////3.1.4.2.3.1. A bytes is similar to byte[], but it is packed tightly in calldata. So bytes should always be preferred over byte[] because it is cheaper
////3.1.4.2.3.2. String is equal to bytes but does not allow length or index access (for now). If you want to access the byte-representation of a string s, use bytes(s).length / bytes(s)[7] = 'x';. Keep in mind that you are accessing the low-level bytes of the utf-8 representation, and not the individual characters!
///3.1.4.2.4.1. Array literals are arrays that are written as an expression and are not assigned to a variable right away.
...
/// 3.1.4.1. comparative operators: <=, <, ==, !=, >=, >
/// 3.1.4.2. bit operators: &, |, ^ (bitwise exclusive or), ~ (bitwise negation)
/// 3.1.4.3. index access: If x is of type bytesI, then x[k] for 0 <= k < I returns the k th byte (read-only).

/// 3.1.4.4. Members of fixed-size byte arrays:
//// 3.1.4.4.1. .length: yields the fixed length of the byte array (read-only).

// 3.1.5 Dynamically-sized byte array: If you can limit the length to a certain number of bytes, always use one of bytes1 to bytes32 because they are much cheaper.
/// 3.1.5.1. bytes: Dynamically-sized byte array. Not a value-type! use bytes for arbitrary-length raw byte data 
/// 3.1.5.2. string: Dynamically-sized UTF8-encoded string. Not a value-type! use string for arbitrary-length string (utf-8) data.

// 3.1.6 Fixed Point Numbers
/// Coming soon

// 3.1.7 Rational and interger literals: All number literals retain arbitrary precision until they are converted to a non-literal type (i.e. by using them together with a non-literal type). Thus computations do not overflow (f.i.2**800 + 1 - 2**800 results in the constant 1 (of type uint8) although intermediate results would not even fit the machine word size) and also divisions do not truncate (.5 * 8 results in the integer 4 although non-integers were used in between). 
...
/// 3.1.7.1. comparative operators: <=, <, ==, !=, >=, >
/// 3.1.7.2. bit operators: &, |, ^ (bitwise exclusive or), ~ (bitwise negation). If any of the rational and interger literals is fractional (x = 1/5) bit operations are disallowed.
/// 3.1.7.3. arithmetic operators: +, -, unary -, unary +, *, /, % (remainder), ** (exponentiation). The exponentiation is disallowed if the exponent is fractional (because that might result in a non-rational number).

// 3.1.8 String Literals: String Literals are written with double quotes ("abc"). As with integer literals, their type can vary, but they are implicitly convertible to bytes if they fit, to bytes and to string.
